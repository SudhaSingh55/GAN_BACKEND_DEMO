...
app.get('/cities-by-tag', verifyToken, (req, res) => {
    ...
    jwt.verify(req.token,'secretKey',(err,authData)=>{
        ...
        fs1.readFile("./addresses.json", "utf8", (err, jsonString) => {
            if (err) {
            ...
            try {
                ...
                const result = cities.filter( obj => obj.tags.indexOf(req.query.tag) >= 0  && String(obj.isActive) === isActive);
                console.log(result);
                var respArray = { 'cities':  result};
    ...
    app.get('/distance', verifyToken, (req, res) => {
        ...
        fs1.readFile("./addresses.json", "utf8", (err, jsonString) => {
            ...
            try {
                const cities = JSON.parse(jsonString);
                const fromObj = cities.filter( obj => obj.guid === req.query.from);
                const toObj = cities.filter( obj => obj.guid === req.query.to);
                const distance = getDistanceFromLatLonInKm(fromObj[0].latitude, fromObj[0].longitude, toObj[0].latitude, toObj[0].longitude);
                var number = Math.round((distance)*100)/100;
